...
class DataValidation:
    ...
    def is_column_exist(self, df: DataFrame) -> bool:
        ...
        try:
            ...
            for column in self._schema_config["date_columns"]:
                if column not in dataframe_columns:
                    missing_date_columns.append(column)

            if len(missing_date_columns)>0:
                logging.info(f"Missing date column: {missing_date_columns}")

            return False if len(missing_categorical_columns)>0 or len(missing_numerical_columns)>0 or len(missing_date_columns)>0 else True
        except Exception as e:
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        ...
        try:
            ...
            if not status:
                validation_error_msg += f"Columns are missing in validation dataframe. "
            else:
                logging.info(f"All required columns present in validation dataframe: {status}")
